...
if ($ci == $_SESSION['ci']) {
    echo "<script>alert('No puede eliminar su propia cuenta.');window.location='read.all.usuario.php';</script>";
    exit();
}

$r = $conexion->query("SELECT COUNT(*) AS total FROM pedidos WHERE vendedor_ci='$ci'")->fetch_assoc();
if ($r['total'] > 0) {
    echo "<script>alert('No se puede eliminar porque tiene pedidos registrados. Puede bloquearlo.');window.location='read.all.usuario.php';</script>";
    exit();
}

$conexion->query("DELETE FROM usuario WHERE ci='$ci'");
header("Location: read.all.usuario.php");
exit();
?>
